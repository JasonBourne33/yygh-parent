...
//                                             ])
            container('maven') {
              git(url: 'https://gitee.com/jasonbourne233/yygh-parent', credentialsId: 'gitee-id', branch: 'master', changelog: true, poll: false)
              sh 'ls -al'
            }
...
//                 }
        ...
        stage('5 deploy to dev') {
            parallel {
                stage('5 hospital-manage - 部署到dev环境') {
                    agent none
                    steps {
...
//         stage('6 send confirm mail') {
//             agent none
//             steps {
//                 mail(to: '1065807387@qq.com', subject: '构建结果', body: "构建成功了  $BUILD_NUMBER")
//             }
//         }
...
//         DOCKERHUB_NAMESPACE = 'lfy_hello'
        REGISTRY = '193.169.0.4'
        DOCKERHUB_NAMESPACE = 'library' // step 4
